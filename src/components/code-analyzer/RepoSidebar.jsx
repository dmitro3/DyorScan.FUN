...
function FileTreeNode({ node, depth, onFileDoubleClick }) {
...
export function RepoSidebar({
  ...
  return (
    <>
      {isOpen && (
        <div
          className="code-analyzer-scope md:hidden fixed inset-0 bg-black/50 z-40"
          onClick={onClose}
        />
      )}
      ...
      {showHiddenFiles && (
        <div className="code-analyzer-scope fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-white/10 rounded-xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              <p className="text-sm text-zinc-400 mb-4">
                The following files and directories are hidden from the file
                tree to reduce noise and improve performance.
              </p>
              {hiddenFiles.map((file, i) => (
                <div
                  key={i}
                  className="flex items-start justify-between gap-4 p-2 rounded bg-zinc-950/50 border border-white/5 text-sm"
                >
