...
export function FilePreview({ isOpen, filePath, repoOwner, repoName, onClose }) {
  ...
  return (
    <AnimatePresence>
      <div className="code-analyzer-scope fixed inset-0 z-[100] flex items-center justify-center p-4">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          <div className="flex items-center justify-between p-4 border-b border-white/10 bg-zinc-900/80 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              ...
              <h2
                className="text-white font-semibold truncate max-w-md"
                title={filePath || ""}
              >
                {filePath}
              </h2>
          ...
          <div className="p-3 border-t border-white/10 bg-zinc-900/80 backdrop-blur-sm flex items-center justify-between text-xs text-zinc-500">
            ...
            <span>
              {fileInfo?.size ? `${(fileInfo.size / 1024).toFixed(2)} KB` : "0 KB"}
            </span>
          </div>
        </motion.div>
      </div>
